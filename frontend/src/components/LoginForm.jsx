  ...
  const handleSubmit = (event) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const userEmail = subValue.split(":")[0]; // Extract email
        const userRole = subValue.split(":")[1]; // Extract role

        // Store token and username in cookies
        Cookies.set("jwtToken", token, { expires: 1 }); // Token expires in 1 day
        Cookies.set("firstname", userEmail.split("@")[0], { expires: 1 }); // Store only username
