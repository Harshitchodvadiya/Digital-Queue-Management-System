  ...
  useEffect(() => {
    ...
    axios
      ...
      const avgServiceTime = totalServed > 0 ? Math.round((totalServiceTime / totalServed) / 60000) : 0;
       ...
       const totalWaitingTime = completedTokens.reduce((sum, token) => {
        if (token.appointedTime && token.activatedTime) {
          return sum + (new Date(token.activatedTime) - new Date(token.appointedTime));
        }
        return sum;
      }, 0);
      const avgWaitingTime = totalServed > 0 ? Math.round((totalWaitingTime / totalServed) / 60000) : 0;
    ...
    <div className="w-1/3 bg-white shadow-lg rounded-lg p-6">
      ...
      <div className="mt-4 space-y-2">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-xl font-bold text-yellow-600">{summary.avgWaitingTime} min</span>
        </div>
      </div>
    </div>
      ...
      <div className="flex gap-6 mt-6">
          {/* token queue */}
        <div className="bg-white shadow-lg rounded-lg p-8 text-center w-205  h-150 ml-15">
